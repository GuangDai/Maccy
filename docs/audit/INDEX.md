...
>
> 最近更新:**2026-07-13**(E5 前七个 progressive-DI slice 完成:History runtime 服务构造注入并将 live globals 限定在 shared composition factory (`29210900842`)；AppState copy/prewarm/Settings-close 实例路径移除 global 回路 (`ed664b2`,`29211587341`)；`StorageSettingsPane` 由 composition 注入 size reader，关闭 DS-033 (`bd238fe`,`29212071043`)；Footer 五个 global action 闭包替换为 AppState 解释的 closed values (`7d1d3e2`,`29212815681`)；Navigation lead 通过不可变构造输出接到 Preview，`NavigationManager` 不再读取 `AppState.shared` (`38ef0c9`,`29214579841`)；Slideout 自持 lead、弱绑定 panel、注入高度策略，删除 4 个实例级 global lookup (`c9f2273`)；SwiftData persistence 构造注入 `ModelContext`，删除其 15 个 `Storage.shared` 读取并取消 History 的隐式 live backend (`4bcfc47`)；两者联合 full matrix `29215547393`。此前 B2–B5/C3/C4 完成:`History.swift` 978→346 LOC；四个模块分担列表、搜索、投影、命令；legacy writer/adapter 与 legacy Search 删除；value UI effects 消除 `History → AppState.shared`；direct context IO 收敛到 SwiftData persistence adapter；load-limit 一次 transaction/save。C3 `29205361439`,C4 `29206774668`,B2d `29209334126`,最终 seam cleanup `29209585359`。D1 仍保持 measured no-go:不以完整搜索/启动速率换内存。) Earlier **2026-07-12**:D1/D2/D3/C5/C6/E2/XcodeGen M0–M3 完成。Earlier **2026-07-10**:`2026-07-10-history-split-plan/` 的 defer 决策避免了 hollow split；其 forcing gate 后来由 B3/B4+B5/C3 实际触发。Earlier 2026-07-09:`2026-07-09-design-audit-verification/` — **27/34 findings confirmed; 6 severity-overstated; DS-019 refuted; 19 new issues**。
...
### 实时摄取路径
- 实时 per-copy 路径:`Clipboard`(pasteboard + `IngestPolicy` 值快照)→lossless FIFO `IngestMailbox`→`BackgroundClipboardIngestor`→`History.consume`→`reconcileWithStore`。D3 令 burst 共享一个 drain Task 且不丢请求；D2 后普通 file/plain/image 不回 main，只有 planner 选中的小型 RTF/HTML 使用 AppKit main 解析。4.4a 经 `model(for:)` + 二分插入增量;G-copy 9.34→0.99ms。`findSimilarItem` / `History.add` 在生产中已死。
...
| [`2026-07-12-e2-package-organization-plan.md`](2026-07-12-e2-package-organization-plan.md) | A | E2/DS-026/032/034 completion record: Application/Search packages, lazy composition, whole-file DEBUG hooks, generated artifact, and full green matrix/package evidence. |
| [`2026-07-12-c6-stored-item-identity-plan.md`](2026-07-12-c6-stored-item-identity-plan.md) | A | C6/DS-019 completion record: Apple stable-ID evidence, no-schema decision, TDD red, direct stored identity, generic pure index, and full green matrix. |
| [`../../CONTEXT.md`](../../CONTEXT.md) | A/domain | Canonical clipboard-history language: History Item, Stored Item Identity, Presentation Identity, Content Signature/Fingerprint, Copy Origin. |
