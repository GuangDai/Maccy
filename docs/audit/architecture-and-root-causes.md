...
| `AppState` runtime boundary | ✅ E5 第二 slice (`ed664b2`):空选择 query copy 构造注入；prewarm 使用当前组合的 History；Settings close observer 弱捕获 owning AppState，不再回写 `AppState.shared`。文件内 shared 使用 8→3，剩余仅 composition/Settings 构建区；full matrix `29211587341`。后续 view-ownership slice 保留 `ContentView` 为 popup 状态 composition root，`HeaderView`/`ToolbarView` 改由 SwiftUI environment 接收同一实例，不再各自抓取 global。 |
