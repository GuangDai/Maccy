...
| Slideout runtime boundary | ✅ Preview 自持 current lead、弱绑定 panel window；`SlideoutController` 的实例级 `AppState.shared` 读取归零，窗口绑定留在 CompositionRoot。侧栏开关只改变水平几何并保留 panel 当前高度，不再依赖或调用 Popup 的纵向高度策略；图片继续在稳定高度内 aspect-fit，源图尺寸不决定主 UI 行高或窗口高度。VF-06 收尾又将 divider drag 内聚为 controller-owned transaction：拖动帧只更新 observable backing widths，结束时 content/preview 各持久化一次，View 不再每帧写 Defaults；full matrix `29298867332`（390 unit）。 |
| Updater runtime boundary | ✅ `SoftwareUpdater` 构造 Sparkle controller 时保持 stopped；自身的幂等 start gate 只在自动检查重新启用或用户显式检查时启动 updater。仅打开 General Settings 且自动检查关闭不会唤起 updater，也不增加启动路径工作。 |
