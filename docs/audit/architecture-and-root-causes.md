...
| Slideout runtime boundary | ✅ Preview 自持 current lead、弱绑定 panel window；`SlideoutController` 的实例级 `AppState.shared` 读取归零，窗口绑定留在 CompositionRoot。侧栏开关只改变水平几何并保留 panel 当前高度，不再依赖或调用 Popup 的纵向高度策略；图片继续在稳定高度内 aspect-fit，源图尺寸不决定主 UI 行高或窗口高度。 |
