...
- [ ] **7.13 Combine/Observation 线程收敛(06-F09/F21)** —【2026-07-03 延后；2026-07-14 decorator 部分落地:`HistoryItemDecorator` 的 `title` 改为直接读取 `item.title`，pin shortcut 在 `HistoryMutations.togglePin` 保存成功后显式同步，`synchronizeItemPin/Title` 两条自重装 relay 删除。专项测试锁定同一 main-actor turn 的 title 投影与 `togglePin` 返回前的 shortcut；RED run `29264161531` 仅这 2 条失败，GREEN full matrix `29264760355` attempt 2 全绿。此前“computed 属性传播无 runner 证据”的延后前提已由真实 runner 消除；`AppDelegate.synchronizeMenuIconText` 单实例 relay 仍保留，因此 7.13 不标完成。】
