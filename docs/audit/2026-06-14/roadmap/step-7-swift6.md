...
> 完成: BS-7(2026-07-14:14/17 — Swift 6.0 complete mode 上线,零 @unchecked/nonisolated(unsafe);7.13 的 decorator 与 status-item recursive relay 已全部替换为显式单向投影)
...
- [x] **7.13 Combine/Observation 线程收敛(06-F09/F21)** —【2026-07-14 完成。`HistoryItemDecorator.title` 直接读取 `item.title`，pin shortcut 在 `HistoryMutations.togglePin` 保存成功后显式同步，`synchronizeItemPin/Title` 两条自重装 relay 已删除；RED `29264161531` 仅 2 条新断言失败，GREEN `29264760355` attempt 2 全绿。最后的 `AppDelegate.synchronizeMenuIconText` 改为 `HistoryListState → AppState.menuIconText → status item` 同步单向 sink，列表每次已完成 publication 立即投影，无 recursive re-arm/异步合并窗口。`ObservationMirrorTests` 连续发布 100 次并断言 101 次有序输出（含初始值）与最终 `copy-99`。分支 full matrix `29301008565`，master auto CI `29301400111`，最终目标 master CI `29303604431` 均绿。】
