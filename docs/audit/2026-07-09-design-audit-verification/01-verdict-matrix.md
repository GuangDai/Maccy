...
| DS-006 | ✅ `△over` | High→**Med** | High | E5 ongoing; History/AppState/Navigation/Slideout/persistence slices injected | Structural/testability only; History runtime globals are confined to composition (`29210900842`), AppState instance paths no longer use global services (`29211587341`), Navigation publishes lead changes through a constructor output (`29214579841`), Slideout owns its lead/window inputs, and SwiftData persistence owns its context (`c9f2273`, `4bcfc47`, joint `29215547393`); other modules remain. |
