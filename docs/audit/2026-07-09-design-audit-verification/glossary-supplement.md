...
| **Mutating read** | A query whose stated purpose is to read (dedup lookup, load) but which writes as a side effect. Candidate fingerprint backfill was moved out of lookup on 2026-07-11; the cold-load trim still deletes overflow rows. | `NEW-ingest-dualpath-2` resolved by `f9f0e85`; the remaining example is `History.load()` → `limitHistorySize(to:)`. The unrelated timestamp self-assignment finding was resolved by `91d76b8`. |
