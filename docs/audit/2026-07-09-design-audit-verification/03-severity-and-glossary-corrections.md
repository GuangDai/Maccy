...
| **"Live ingest" alias** | **Baseline correction remains valid. Current (`a487276`):** no unconditional hop remains; the request carries a live policy snapshot and only planner-selected small RTF/HTML parsing enters `MainActor` (AppKit affinity). |
