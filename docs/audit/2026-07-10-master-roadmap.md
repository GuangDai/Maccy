...
| E5 | Progressive DI vs `shared` | DS-006 | ongoing | M | First slice complete: History runtime effects are constructor-injected; ordinary instances are inert and projector tests record event batches without the global ingestor (`c8c8fbf`–`02711cb`, run `29210900842`). Continue boundary-by-boundary; don't big-bang. |
